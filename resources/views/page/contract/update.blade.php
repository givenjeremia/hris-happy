...
@section('toolbar')
    @include('components/toolbar', ['title' => 'Update', 'subtitle' => 'Contract'])
@endsection

@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Form Update Contract</h3>
        </div>
        <form id="formUpdate">
            @csrf
            @method('put')
            <div class="card-body">
                <div class="form-group required">
                    ...
                    <select name="client" class="form-control">
                        @foreach ($clients as $item)
                            <option value="{{ $item->uuid }}" {{ $item->id == $contract->client_id ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group required">
                    ...
                    <input type="date" name="start_date" class="form-control" id="exampleInputEmail1" value="{{ $contract->start_date }}" placeholder="Write Data">
                </div>
                <div class="form-group required">
                    <label for="exampleInputEmail1" class="control-label">End Date</label>
                    <input type="date" name="end_date" class="form-control" id="exampleInputEmail1" value="{{ $contract->end_date }}" placeholder="Write Data">
                </div>
                ...
                <div class="form-group ">
                    <iframe src="{{ $url }}" id="pdf-preview" class="mb-2 w-100" style="height: 500px;" src="" frameborder="0"></iframe>
                </div>
            </div>
        ...
        $('#btn-simpan').click(function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let act = '{{ route('contracts.update', ":uuid" )}}'.replace(':uuid','{{ $contract->uuid}}')
